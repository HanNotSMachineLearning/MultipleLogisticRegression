...
while True:
    print("\nGeslacht? (0 voor MAN, 1 voor VROUW)")
    ...
    prediction = DT_clf.predict([[geslacht, leeftijd, hoesten, zere_keel, pijn_op_de_borst,
                                  kortademigheid, druk_op_de_borstkas, koorts, snelle_ademhaling]])[0]
    # link correct fruitname to predicted value
    print("\n\nI think you have a:")
    print("\t\t" + prediction)
    time.sleep(1)
    print("\n\n\nDo you want me to predict LONGONTSTEKING again? (Y/N)")
    again = input("")
    if again.upper() != "Y":
        break
